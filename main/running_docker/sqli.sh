...
BACKUP_DOCKER_COMPOSE_PATH="/var/www/html/dockers-backup/docker2/docker-compose.yml"
BACKUP_CONFIG_PATH="/var/www/html/dockers-backup/docker2/config"
...
cleanup_unused_docker() {
    ...
    for dir in $BASE_DIR/docker_sqli_*; do
...
prompt_for_ports() {
    ...
    echo "Received ports: $BASE_PORT_1 and $BASE_PORT_2 for checking."
    ...
    echo "Ports $BASE_PORT_1 and $BASE_PORT_2 passed all checks."
}
...
create_and_start_container() {
    ...
    mkdir -p "$BASE_DIR"
    local i=1
    while [ -d "$BASE_DIR/docker_sqli_$i" ] || does_container_exist "docker_sqli_$i"; do
        ((i++))
    done
    local NEW_DIR="$BASE_DIR/docker_sqli_$i"
    local NEW_CONTAINER_NAME="docker_sqli_$i"

    # Correctly map the user-specified ports
    ...
    # Use sed to replace the ports in docker-compose.yml
    sed -i "s/3002:3000/${BASE_PORT_1}:3000/g" "$NEW_DIR/docker-compose.yml"
    sed -i "s/3003:3001/${BASE_PORT_2}:3001/g" "$NEW_DIR/docker-compose.yml"
    sed -i "s/container_name: docker2/container_name: $NEW_CONTAINER_NAME/g" "$NEW_DIR/docker-compose.yml"
